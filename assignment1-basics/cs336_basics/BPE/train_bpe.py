...
def init_vocab(vocab_size: int, special_tokens: list[str])->dict[int, bytes]:
    if vocab_size < 256 + len(special_tokens):
        raise ValueError("vocab_size 小于“初始 256 bytes 加 special tokens 所需的最小词表大小")
    return {i:bytes([i]) for i in range(256)} | {(256+i):special_tokens[i].encode("utf-8") for i in range(len(special_tokens))}

def train_bpe(vocab_size: int, vocab_table:dict[int, bytes], frequency_table:dict[tuple[bytes, ...], int]) -> tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
    ...
    for _ in range(merge_round):
        ...
        for word, frequency in frequency_table.items():
            for pair_index in range(len(word) - 1):
                ...
                pair_frequency_table[bind] = pair_frequency_table.get(bind, 0) + frequency
        if bind_flag == False:
            break
        ...
        for word, frequency in frequency_table.items():
            ...
            if pair_index == len(word) - 1:
                new_word += (word[pair_index],)
            new_frequency_table[new_word] = new_frequency_table.get(new_word, 0) + frequency
        
        frequency_table = new_frequency_table
    return (vocab_table, merges)

def train_bpe_from_filepath(input_path: str | os.PathLike, vocab_size: int, special_tokens: list[str], num_processes: int) -> tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
    vocab_table:dict[int, bytes] = init_vocab(vocab_size, special_tokens)
    frequency_table:dict[tuple[bytes, ...], int] = pretokenize(input_path, special_tokens, num_processes)
