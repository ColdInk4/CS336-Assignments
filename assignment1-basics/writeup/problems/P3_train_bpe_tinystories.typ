...
a. 大约用了 17 秒；用 /usr/bin/time -v 测得峰值内存为 194724 kB，约等于 195 MB RSS。词表中最长的 token 是 b' accomplishment'，长度为 15 bytes，这很合理，因为 TinyStories 中这类带前导空格的高频英文词片段确实容易被合并成一个 token。

b. 从 profiling 结果看，训练时间主要花在 pre-tokenization 阶段，而不是 BPE merge 主循环。也就是说，最耗时的部分是对语料分块、正则预分词和统计 pretoken 频次，而 merge 阶段相对只占很小一部分时间。
]
]
